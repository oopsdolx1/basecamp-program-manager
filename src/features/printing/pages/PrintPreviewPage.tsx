...
import { Button, Card, EmptyState, Loading, StatusChip, colors, motion, radius, shadows, spacing } from "../../../design-system";
...
export const PrintPreviewPage = (): JSX.Element => {
  ...
  const steps = ["운동 세션", "QR 생성", "미리보기", "출력 준비"];
        ...
        <Stack spacing={`${spacing[3]}px`}>
          ...
          <Card variant="analysis">
            <Stack alignItems={{ md: "center", xs: "flex-start" }} direction={{ md: "row", xs: "column" }} justifyContent="space-between" spacing={`${spacing[3]}px`}>
              <Box><Typography color={colors.primary.gold} fontWeight={800} variant="overline">출력 세션</Typography><Typography fontFamily="inherit" fontWeight={700} letterSpacing="-0.02em" lineHeight={1.3} variant="h4">오늘의 운동</Typography></Box>
              <Stack direction={{ sm: "row", xs: "column" }} spacing={`${spacing[2]}px`} sx={{ width: { xs: "100%", sm: "auto" } }}>
                <Button variant="secondary" startIcon={<ArrowBackIcon />} onClick={goWorkspace}>프로그램 변경</Button>
          ...
          <Card>
            <Box sx={{ display: "grid", gap: `${spacing[3]}px`, gridTemplateColumns: { lg: "repeat(4, minmax(0, 1fr))", sm: "repeat(2, minmax(0, 1fr))", xs: "1fr" } }}>
              ...
              <Box><Typography color={colors.neutral.gray400} variant="caption">프로그램</Typography><Typography fontWeight={800}>{state.document.program.title}</Typography></Box>
              <Box><Typography color={colors.neutral.gray400} variant="caption">운동 세션</Typography><Typography fontFamily="monospace" fontWeight={800} sx={{ overflowWrap: "anywhere" }}>{state.document.workoutSessionId}</Typography></Box>
